...
class DQNTest(Node):
    ...
    def __init__(self, stage, load_episode):
        ...
        self.state_size = 3 #26
        self.action_size = 6
        self.episode_size = 510
        ...
        self.model_dir_path = os.path.join(
            '/home/dockeruser/project/dev_ws/src/chamoe_rl',
            'saved_model'
        )
        
        self.model_path = os.path.join(
            self.model_dir_path,
            'DQN4_stage1_episode'+str(self.load_episode)+'.h5')
        ...
        if self.load_model:
            ...
            with open(os.path.join(
                    self.model_dir_path,
                    'DQN4_stage'+str(self.stage)+'_episode'+str(self.load_episode)+'.json')) as outfile:
    ...
    def process(self):
        ...
        total_step = 0
        success_count = 0
        episode_count = self.episode_size
        score_list = []
        for episode in range(self.load_episode+1, self.episode_size):
            ...
            self.get_logger().info(f'Episode {episode} finished – score: {score}, steps: {local_step}')
            total_score += score
            score_list.append(score)
            total_step += local_step
            if score >= 1500:  # 예: score > 100
                success_count += 1

        print(f"평균 score: {total_score/episode_count}")
        print(f"score_list: {score_list}")
        print(f"평균 step: {total_step/episode_count}")
    ...
    def get_action(self, state):
        if numpy.random.rand() <= self.epsilon:
            ...
            self.action_pub.publish(Float32MultiArray(data=[numpy.argmax(q_value[0])]))
