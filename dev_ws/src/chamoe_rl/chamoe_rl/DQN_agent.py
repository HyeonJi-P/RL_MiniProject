...
class DQNAgent(Node):
    ...
    def __init__(self, stage_num, max_training_episodes):
        ...
        raw_state = self._get_initial_state()  # Fetch raw state first
        self.state_size = len(raw_state)       # Set state_size
        print(f"[DQNAgent] state_size: {self.state_size}")  # Debug print
        initial_state = numpy.reshape(numpy.asarray(raw_state), [1, self.state_size])  # Now reshape
        #self.state_size = 26
        self.action_size = 6
            ...
            if self.train_mode:
                if episode % 100 == 0:
                    ...
                    with open(
                        os.path.join(
                            self.model_dir_path,
                            'DQN_stage' + str(self.stage) + '_episode' + str(episode) + '.json'
                        ),
                        'w'
                    ) as outfile:
    ...
    def reset_environment(self):
        ...
        if future.result() is not None:
            ...
            print(f"[reset_environment] state len: {len(state)}")
            if len(state) != 5:
                raise ValueError(f"State length is {len(state)}, but 3 is expected! State: {state}")
    ...
    def step(self, action):
        ...
        if future.result() is not None:
            ...
            print(f"[step] next_state len: {len(next_state)}")
            if len(next_state) != 5:
                raise ValueError(f"Next state length is {len(next_state)}, but 3 is expected! State: {next_state}")
    ...
    def create_qnetwork(self):
        model = Sequential()
        model.add(Dense(512, input_shape=(self.state_size,), activation='relu'))  # self.state_size=3
